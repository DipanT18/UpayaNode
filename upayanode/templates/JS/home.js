...
// ── Session Node ID (would come from auth in production) ──
const MY_NODE_ID = 'Node #' + (localStorage.getItem('un_node') || generateSessionId());
function generateSessionId() {
  const id = Math.floor(1000 + Math.random() * 9000);
  try { localStorage.setItem('un_node', id); } catch(e){}
  return id;
}
...
const SEED_POSTS = [
  {
    id: 'p1',
    nodeId: 'Node #3842',
    text: 'There\'s a huge pothole at the junction near the school on Main Street. Multiple bikes have fallen. No one seems to be reporting it to the municipality. What\'s the fastest way to get this fixed?',
    media: null,
    time: '2h ago',
    solves: [
      { nodeId: 'Node #7201', text: 'File a complaint directly on your city\'s municipal portal. Most cities have an online grievance system. Attach a photo for faster action.', time: '1h ago', mine: false },
      { nodeId: 'Node #5590', text: 'You can also tweet at your local corporator\'s official handle with the photo. Public pressure on social media often gets faster response than formal channels.', time: '45m ago', mine: false },
    ]
  },
  {
    id: 'p2',
    nodeId: 'Node #6117',
    text: 'Water supply in our area has been irregular for 3 weeks. We get water only every alternate day and sometimes not even that. Tankers are charging absurd rates. How do people deal with this?',
    media: null,
    time: '5h ago',
    solves: [
      { nodeId: 'Node #2234', text: 'Contact your ward councillor directly — this falls under their responsibility. Keep a log with dates and times to show the pattern.', time: '4h ago', mine: false },
    ]
  },
  {
    id: 'p3',
    nodeId: 'Node #9053',
    text: 'My landlord is refusing to return the security deposit even after I vacated 2 months ago. Has made multiple excuses. What legal options do I have without spending too much on a lawyer?',
    media: null,
    time: '1d ago',
    solves: [
      { nodeId: 'Node #4480', text: 'Send a legal notice via a registered letter first. Templates are available online. This costs very little and is often enough to get landlords moving.', time: '23h ago', mine: false },
      { nodeId: 'Node #8812', text: 'You can file in consumer court if the amount qualifies. Rent disputes up to a threshold can be filed without a lawyer in most states.', time: '20h ago', mine: false },
      { nodeId: 'Node #1193', text: 'Also document all your communications — screenshots of messages, emails. These serve as evidence if it escalates.', time: '18h ago', mine: false },
    ]
  },
  {
    id: 'p4',
    nodeId: 'Node #2278',
    text: 'Street dogs near our apartment have become aggressive. One bite incident happened last week. The RWA is not acting. Who has the authority to address this?',
    media: null,
    time: '3d ago',
    solves: []
  },
...
// Submit post
function submitPost(text, media) {
  const newPost = {
    id: 'p' + Date.now(),
    nodeId: MY_NODE_ID,
    text: text.trim(),
...
mobilePostForm.addEventListener('submit', (e) => {
  e.preventDefault();
  const text = mobilePostText.value.trim();
  if (text.length < 10) return;

  submitPost(text, mobileMediaFile);
