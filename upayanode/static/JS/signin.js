...
signInBtn.addEventListener('click', async () => {
  ...
  try {
    const res = await fetch(window.location.pathname, {
      ...
      body: JSON.stringify({ passphrase: raw }),
    });
    ...
    if (res.ok && data.success) {
      ...
      nodeIdEl.textContent = data.node_id || generateNodeId();
    } else {
      signInBtn.innerHTML = 'Enter UpayaNode <span>→</span>';
      signInBtn.disabled = false;
      passphraseInput.classList.add('shake');
      inputHint.textContent = data.error || '✗ Passphrase not recognised';
      inputHint.className = 'input-hint err';
    ...
    signInBtn.innerHTML = 'Enter UpayaNode <span>→</span>';
    signInBtn.disabled = false;
    inputHint.textContent = 'Network error. Try again.';
    inputHint.className = 'input-hint err';
  }
});
